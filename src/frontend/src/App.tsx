    ...
    <>
      <div
        ...
        <div className="flex flex-col justify-center items-center absolute top-16">
          ...
          <p className="text-sm mt-2 mb-6">made by <a
            className="cursor-pointer hover:underline transition">@Satriadhikara</a></p>
        </div>
        {!data ? (<>
          <Form {...form}>
            ...
            {data?.path?.map((word, index) => (
              <div key={index} className="flex gap-3">
                {word.split('').map((letter, letterIndex) => {
                  const endWordLetter = form.getValues().endWord[letterIndex];
                  const bgColor = letter !== endWordLetter ? 'bg-slate-700' : 'bg-white';
                  const textColor = letter !== endWordLetter ? 'text-white' : 'text-slate-700';
                  return (
                    <p key={letterIndex}
                       className={`rounded h-20  w-14 flex justify-center items-center text-5xl ${bgColor} ${textColor}`}>{letter}</p>
                  );
                })}
              </div>
            ))}
